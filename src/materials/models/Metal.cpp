...
Metal::Metal(Texture* const reflection, Texture* const roughness): Material(), reflection(reflection), roughness(roughness) {}
...
Light Metal::color(World const& world, Ray const& in, RelativePosition const& position, int const samples, int const depth) const {
	...
	auto l = f * specular(world, in, d, r, true, samples, depth).addId(this);
	return l;
}